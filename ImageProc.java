// P5 - Real Image Processing | ImageProc
// Edward Ayala
...
public class ImageProc {
    ...
    public static class Image {
        ...
        public BufferedImage filterImageBlur(float [] matrix){
            BufferedImage img= new BufferedImage(rows, cols, BufferedImage.TYPE_INT_RGB ); // create buffimg
            ...
            // BufferedImageOp - makes it possible to perform operations on the BufferedImage instance
        ...
        public void blurImage() {
            ...
            BufferedImage img = filterImageBlur(blurMatrix);
            int[][] arr = new int[rows][cols];
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
        ...
        // filter for emboss
        public BufferedImage filterImageEmboss(float [] matrix){
            BufferedImage img= new BufferedImage(rows, cols, BufferedImage.TYPE_INT_RGB ); // create buffimg
            // fill image with pixel values
            int [][] arr = this.getPixels();
            for (int i=0; i < rows; i++){
            ...
            // filter image through emboss filter
            // BufferedImageOp - makes it possible to perform operations on the BufferedImage instance
            BufferedImageOp emboss = new ConvolveOp( new Kernel(3, 3, matrix));
            BufferedImage imgDst = emboss.createCompatibleDestImage(img, null);
            img = emboss.filter(img, imgDst);
            return img;

            //using Image class - not using BufferedImageOp
//            int [][] p = this.getPixels();
//
...
//                    {0,  1, 2,}
//            };
            // method using BufferedImageOp to filter
            float [] embossMatrix = {
                    -2, -1, 0,
                    -1,  1, 1,
                     0,  1, 2,
            };
            BufferedImage img = filterImageEmboss(embossMatrix);
            int [][] arr = new int [rows][cols];
           // int [][] img = filterImageEmboss(embossMatrix);
            for (int i = 0; i < rows; i++) {
                for (int j = 0; j < cols; j++) {
                    arr[i][j] = img.getRGB(i, j);
                    pixels[i][j].setValue(arr[i][j]);
